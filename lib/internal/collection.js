...
function createEachSeriesWithKey(createResult, createCallbackHandler) {
  ...
  function callArrayEach(array, iterator) {
    ...
    return new Promise((resolve, reject) => {
      let i = -1;
      const createCallback = createCallbackHandler(l, resolve, result, iterate, array);
      iterate();
      function iterate() {
        const promise = call3(iterator, array[++i], i, array);
        if (promise === errorObj) {
  ...
  function callObjectEach(object, iterator) {
    ...
    return new Promise((resolve, reject) => {
      let i = -1;
      const createCallback = createCallbackHandler(l, resolve, result, iterate, object, keys);
      iterate();
      function iterate() {
        const key = keys[++i];
        const promise = call3(iterator, object[key], key, object);
