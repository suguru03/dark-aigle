...
parallel('sortByLimit', () => {
  ...
  it('should execute', async () => {
    ...
    const res = await Promise.sortByLimit(collection, 2, iterator);
  ...
  it('should execute with default concurrency which is 8', async () => {
    ...
    Promise.sortByLimit(collection, iterator);
  ...
  it('should return an empty array if collection is an empty array', async () => {
    ...
    const res = await Promise.sortByLimit([], iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
  ...
  it('should return an empty array if collection is string', async () => {
    ...
    const res = await Promise.sortByLimit('test', iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
    assert.strictEqual(res.length, 0);
  });
  ...
  it('should stop execution if error is caused', async () => {
    ...
    try {
      await Promise.sortByLimit(collection, 2, iterator);
      assert.fail();
    } catch (e) {
...
parallel('#sortByLimit', () => {
  ...
  it('should execute', async () => {
    ...
    const res = await Promise.resolve(collection).sortByLimit(2, iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
    assert.deepEqual(res, [1, 2, 3, 4, 5]);
    assert.deepEqual(order, [
      [0, 1],
  ...
  it('should execute with delay', async () => {
    ...
    const res = await Promise.delay(DELAY, collection).sortByLimit(2, iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
    assert.deepEqual(res, [1, 2, 3, 4, 5]);
    assert.deepEqual(order, [
      [0, 1],
  ...
  it('should execute with default concurrency which is 8', async () => {
    ...
    Promise.resolve(collection).sortByLimit(iterator);
    await Promise.delay(DELAY * 5);
    assert.deepEqual(order, _.times(8));
  });
});
