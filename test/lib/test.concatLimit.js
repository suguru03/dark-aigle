...
parallel('concatLimit', () => {
  ...
  it('should return an empty array if collection is an empty array', async () => {
    ...
    const res = await Promise.concatLimit([], iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
  ...
  it('should return an empty array if collection is string', async () => {
    ...
    const res = await Promise.concatLimit('test', iterator);
    assert.strictEqual(Object.prototype.toString.call(res), '[object Array]');
    assert.strictEqual(res.length, 0);
  });
  ...
  it('should stop execution if error is caused', async () => {
    ...
    try {
      await Promise.concatLimit(collection, 2, iterator);
      assert.fail();
    } catch (e) {
